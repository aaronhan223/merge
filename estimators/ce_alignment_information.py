...
def simple_discrim(xs, y, num_labels, device=None):
    if device is None:
...
class CEAlignment(nn.Module):
    ...
    def forward(self, x1, x2, x1_probs, x2_probs):
        ...
        for i in range(align.size(-1)):
            current = align[..., i]
            # Balance marginal masses so Sinkhorn can converge (minibatch sums differ).
            m1 = x1_probs[:, i].sum()
            m2 = x2_probs[:, i].sum()
            x2_i = x2_probs[:, i] * (m1 / (m2 + 1e-8))
            for j in range(500):
                current, stop = sinkhorn_probs(current, x1_probs[:, i], x2_i)
                if stop:
                    break
...
class CEAlignmentInformation(nn.Module):
    ...
    def forward(self, x1, x2, y):
        ...
        log_p_y = torch.log(self.p_y + 1e-8)
        mi_y_x1 = torch.mean(torch.sum(p_y_x1 * (torch.log(p_y_x1 + 1e-8) - log_p_y[None]), dim=-1))
        mi_y_x2 = torch.mean(torch.sum(p_y_x2 * (torch.log(p_y_x2 + 1e-8) - log_p_y[None]), dim=-1))
        mi_y_x1x2 = torch.mean(torch.sum(p_y_x1x2 * (torch.log(p_y_x1x2 + 1e-8) - log_p_y[None]), dim=-1))
...
def train_discrim(model, train_loader, optimizer, data_type, num_epoch=40, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    for _iter in tqdm(range(num_epoch), desc="Training Discriminator"):
        for i_batch, data_batch in enumerate(train_loader):
            ...
            for j in range(len(data_type)):
                ...
                x_batch = torch.cat(xs, dim=-1).to(device)
...
def eval_discrim(model, test_loader, data_type, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    losses = []
    for i_batch, data_batch in enumerate(tqdm(test_loader, desc="Evaluating Discriminator")):
        inputs = []
        for j in range(len(data_type)):
            xs = [data_batch[data_type[j][i] - 1] for i in range(len(data_type[j]))]
            x_batch = torch.cat(xs, dim=-1).to(device)
...
def train_ce_alignment(model, train_loader, opt_align, data_type, num_epoch=10, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    for _iter in tqdm(range(num_epoch), desc="Training CE Alignment"):
        for i_batch, data_batch in enumerate(train_loader):
            opt_align.zero_grad()
            ...
            x1_batch = torch.cat(x1s, dim=-1).float().to(device)
            x2_batch = torch.cat(x2s, dim=-1).float().to(device)
            y_batch = torch.cat(ys, dim=-1).to(device)

            loss, _, _ = model(x1_batch, x2_batch, y_batch)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.align_parameters(), max_norm=1.0)
            opt_align.step()

def eval_ce_alignment(model, test_loader, data_type, device=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for i_batch, data_batch in enumerate(tqdm(test_loader, desc="Evaluating CE Alignment")):
        x1s = [data_batch[data_type[0][i] - 1] for i in range(len(data_type[0]))]
        x2s = [data_batch[data_type[1][i] - 1] for i in range(len(data_type[1]))]
        ys = [data_batch[data_type[2][i] - 1] for i in range(len(data_type[2]))]
